...
>-------------------------------------------------------
> - Testing dataset with K Means clustering classifier:
>-------------------------------------------------------
>	Accuracy: 0.58946	Precision: 0.14080	Recall: 0.32700	F1: 0.19684	F2: 0.25860
...
Nesta próxima etapa, é importante a aplicação de técnicas visando obter melhoria dos resultados obtidos por meio da seleção das features efetivamente relevantes como também, os melhores parâmetros para o algoritmo selecionado (DecisionTree).

Para isso, foi utilizado um recurso chamado GridSearchCV, que possibilita "varrer" os parâmetros que se deseja melhorar até atingir o melhor resultado da métrica especificada.

Utilizou-se assim, o recurso SelectKBest para seleção das melhores features e os alguns dos parâmetros que mais influenciam os resultados de uma DecisionTree (criterion, splitter, min_samples_split, min_samples_leaf e max_depth). A métrica selecionada foi a F1, pois por meio da melhoria desta métrica espera-se obter melhores resultados tanto para a métrica Precision, quanto para a métrica Recall.
